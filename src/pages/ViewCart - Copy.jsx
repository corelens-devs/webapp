...
const ViewCart = () => {
  ...
  const processPayment = async () => {
    ...
    try {
      ...
      const paymentData = {
        mobile_number: phoneNumber.replace(/\D/g, ""), // Clean phone number
        name: formData.name.trim(),
        email: formData.email.trim(),
        city: formData.city.trim(),
        amount: getFinalTotal(), // Backend might expect 'amount' instead of 'total_amount'
        total_amount: getFinalTotal(),
        currency: "INR",
        receipt: `order_${Date.now()}_${phoneNumber}`, // Add receipt for order tracking
      ...
      // Step 1: Create order through backend using simplified structure like working HTML
      console.log("Creating order with simplified payload...");

      const orderPayload = {
        amount: getFinalTotal(),
        products: cartItems.map((item) => ({
          product_id: item.id.split("-")[0], // Extract actual product ID
          quantity: item.quantity,
        })),
        convenience_fee: 0,
        delivery_fee: 0,
        net_amount: getFinalTotal(),
        billing_address_id: "670cc981f710a22e3ba48a93", // Same as working HTML
        payment_method: "Online",
        tax: getGSTAmount(),
        callback_url: `https://backend.corelens.in/api/app/test/paymentVerification?successUrl=${encodeURIComponent(
          "https://thelmm.in/payment-success.php"
        )}&failureUrl=${encodeURIComponent(
          "https://thelmm.in/payment-failure.php"
        )}`,
      };
      ...
      try {
        // ✅ Get fresh token for payment request
        ...
        console.log(
          "🔐 Using dynamic token for payment:",
        ...
        console.error("Order API fetch error:", fetchError);
      ...
      if (!orderData || !orderData.success) {
        const errorMessage = orderData?.message || "Unknown error occurred";
      ...
      // Step 2: Initialize Razorpay payment with simple options like working HTML
      const razorpayOptions = {
        key: orderData.key_id || "rzp_test_RBxTR01ZyYPaUV",
        ...
        handler: async function (response) {
          ...
          try {
            ...
            console.log("Saving order payload:", orderSavePayload);
            ...
            console.log("Order save response:", result);

            if (result.success) {
              alert(
                "✅ Order saved successfully! You can check it in Purchase History."
              );
            } else {
              alert("⚠️ Payment success but order not saved to history.");
            }
          } catch (err) {
            console.error("Error saving order:", err);
            alert("⚠️ Payment succeeded, but failed to save order!");
          }
        },
        modal: {
          ondismiss: function () {
      ...
      // Step 3: Open Razorpay checkout (simplified like working HTML)
