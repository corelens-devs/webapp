...
const Header = () => {
  ...
  useEffect(() => {
    const loadCartFromStorage = async () => {
      try {
        ...
        try {
          ...
          lastUpdated = localStorage.getItem("cartLastUpdated");
        } catch (accessError) {
          return;
        }
          ...
          try {
            ...
            if (Array.isArray(parsedItems) && parsedItems.length > 0) {
              ...
              if (validItems.length > 0) {
                ...
                // Re-save cleaned data if needed
                if (validItems.length !== parsedItems.length) {
                  try {
                    const cleanedData = JSON.stringify(validItems);
                    localStorage.setItem("cartItems", cleanedData);
                ...
                window.dispatchEvent(
                  new CustomEvent("cartUpdated", { detail: validItems }),
                );
              } else {
                setCartItems([]);
                setCartCount(0);
    ...
    setTimeout(loadCartFromStorage, 500);

    // Additional check after mount
    setTimeout(() => {
      const cartCheck = localStorage.getItem("cartItems");
      const isValidCartData = cartCheck &&
        cartCheck !== "[]" &&
        cartCheck !== "null" &&
        cartCheck !== "undefined" &&
        cartCheck.trim() !== "";

      if (isValidCartData && cartItems.length === 0) {
        loadCartFromStorage();
      }
    }, 1000);
    ...
    const checkLoginStatus = () => {
      try {
        ...
        // Check if login is valid (within 30 days)
        if (
          userToken &&
          userPhone &&
          userVerified === "true" &&
          ...
          if (isValid) {
            ...
            // Try to get user name from saved user info, fallback to phone
    ...
    const handleVisibilityChange = () => {
      if (!document.hidden) {
        const currentCartData = localStorage.getItem("cartItems");
        ...
        if (
          ...
          setTimeout(loadCartFromStorage, 100);
        } else if (!currentCartData && cartItems.length > 0) {
          try {
